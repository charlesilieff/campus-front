            ...
            <tbody>
              {bedList.map((bed, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{bed.room ? <Link to={`room/${bed.room.id}`}>{bed.room.name}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${bed.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">Voir</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${bed.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
                        <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Modifier</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${bed.id}/delete`} color="danger" size="sm" data-cy="entityDeleteButton">

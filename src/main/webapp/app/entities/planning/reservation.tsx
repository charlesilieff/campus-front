...
export const Reservation = ({ reservation, index, place, positionX, positionY, date }: IProps) => {
  const positionYEnd = {};
  ...
  return reservationBedIds.map((bedId: number, indexBed: number) => {
    ...
    if (indexBed < reservation.bedsId.length - 1) {
      if (positionY[bedId] - 1 === positionY[reservation.bedsId[indexBed + 1]]) {
        ...
        positionYEnd[reservation.bedsId[indexBed + 1]] = positionY[reservation.bedsId[indexBed + 1]] + 1;
      }
    }

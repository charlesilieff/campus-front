...
describe('Resservation request e2e test', () => {
  ...
  const requestPageUrlDetail =
    '/gestionhebergement/reservation-request/df53dd96-0177-49ae-b8cd-6499d7e54f37'
  // const customerPageUrlPattern = new RegExp('/customer(\\?.*)?$');
  ...
  const reservationRequestDetailRegex = new RegExp(
    '/reservation-request/df53dd96-0177-49ae-b8cd-6499d7e54f37'
  )
  before(() => {
    cy.window().then(win => {
  ...
  beforeEach(() => {
    cy.intercept('GET', '/api/reservationrequest/df53dd96-0177-49ae-b8cd-6499d7e54f37').as(
      'entitiesRequest'
    )
    cy.intercept('POST', '/api/reservationrequest').as('postEntityRequest')
    cy.intercept('PATCH', '/api/reservationrequest/df53dd96-0177-49ae-b8cd-6499d7e54f37').as(
      'patchEntityRequest'
    )
  })
  ...
  it('should load a reservation request and modify the reservation request', function () {
    ...
    cy.get(`[data-cy="phoneNumber"]`).clear().type('056558898978').should(
      'have.value',
      '056558898978'
    )

    cy.get(`[data-cy="email"]`).clear().type('dalric.Lambert80@yahoo.fr').should(
      'have.value',
      'dalric.Lambert80@yahoo.fr'
    )
    ...
    cy.get(`[data-cy="isArrivalLunch"]`).should('be.visible').check({ force: true }).should(
      'be.checked'
    )
    cy.get(`[data-cy="isDepartureLunch"]`).should('be.visible').check({ force: true }).should(
      'be.checked'
    )
    cy.get(`[data-cy="arrivalDate"]`).clear().type('2022-08-24').should('have.value', '2022-08-24')

    cy.get(`[data-cy="departureDate"]`).clear().type('2022-09-23').should(
      'have.value',
      '2022-09-23'
    )
  ...
  it('should create an instance of Reservation request', () => {
    ...
    cy.get(`[data-cy="email"]`).type('Adalric.Lambert80@yahoo.fr').should(
      'have.value',
      'Adalric.Lambert80@yahoo.fr'
    )
    ...
    cy.get(`[data-cy="comment"]`).type('back-end HTTP enterprise').should(
      'have.value',
      'back-end HTTP enterprise'
    )
    cy.get(entityCreateSaveButtonSelector).click({ force: true })
